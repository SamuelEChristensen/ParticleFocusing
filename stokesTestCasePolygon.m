...
%sol=@(x,k) [x(:,1).^2.*x(:,2), -x(:,2).^2.*x(:,1), zeros(size(x,1),1)]*N([0,0],k);
...
%     fh=@(p) min(0.02+0.5*abs(fd(p)),0.15);
    [p,t]=distmesh2d(@dpoly,@huniform,0.04,[0,0;1.5,1.5],pv,pv);
