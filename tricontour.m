...
%       
      xtmp(k+1)=xtmp(1);                % throw away unused storage
      ytmp(k+1)=ytmp(1);                % xtmp,ytmp contain subcontour
      
      xtmp(k+2:end)=[];                % throw away unused storage
      ytmp(k+2:end)=[];                % xtmp,ytmp contain subcontour
      k=k+1;
...
%                'UserData',zc)
 patch('XData',xtmp,'YData',ytmp,'CData',repmat(zc,k,1),...
               'Parent',hax,'FaceColor','b','EdgeColor','flat',...
               'UserData',zc)
