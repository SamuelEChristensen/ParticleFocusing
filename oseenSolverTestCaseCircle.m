...
f = @(x,k) [zeros(size(x,1),1)...
            ...
            -1i*k*N(x,k).*(x(:,1).^2 + x(:,2).^2 - 2*epsi^2)/epsi^2 + epsi^2*k^2*1i*k*N(x,k)...
              + (1 - x(:,1).^2-x(:,2).^2) .* (k^2*N(x,k))*epsi^2 + x(:,1)*N([0  0], k), ...
            (x(:,2).^2+x(:,1).^2 -epsi^2*4).*x(:,2).*N(x,k)/epsi^4 - k^2*x(:,2).*N(x,k)...
            - 2*1i*k*x(:,2).*N(x,k)*epsi^2 + (1 - x(:,1).^2-x(:,2).^2) .* (1i*k * x(:,2) .* N(x,k)) - 1i*k*x(:,1).*x(:,2)*N([0  0], k)];
