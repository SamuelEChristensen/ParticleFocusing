%test case for oseenSolver with gaussian approximation of delta function
 ...
 fh=@(p) min(0.02+2*abs(dcircle(p,0.5,0,0)).^2,0.1);
 fh=@(p) min(0.02+2*drectangle(p,-0.01,0.01,0,1).^3,0.1);
 [p,t]=distmesh2d(fd,fh,0.02,[-1,-1;1,1],[0,0]);
fh=@(p) min(0.02+2.5*drectangle(p,-0.0,0.0,0,1).^3,0.1);
%fd=@(p) sqrt(sum(p.^2,2))-2;% +1/10*sin(2*pi*p(:,1)).*cos(2*pi*p(:,2));
%[p,t]=distmesh2d(fd,fh,0.01,[-1,-1;1,1],[0  0]);
maxWaveNum = 150;
L = 4;
