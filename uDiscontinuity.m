...
fd=@(p) drectangle(p,0,4,0,1);
fh=@(p) min(max(parami(1), parami(1)+0.4*drectangle(p, 1.99, 2.01, 0.1, 0.5).^2),0.1);
...
toc
Ux = zeros(1000,length(xp));
Uy = zeros(1000,length(xp));
    for j = 1:500
        xpi = [2; 0.5*j/1000];
    ...
    figure
    for j=1:length(xp)
    subplot(2,length(xp),j)
    plot(xvec,Uy(:,j)-4/9*(8*(xp(2,j)-0.5))^2*heaviside(xvec'-xp(2,j)))
    subplot(2,length(xp),j+length(xp))
    plot(xvec,Uy(:,j))
    end
end
