function [velocity,p,t] = DisturbanceFieldSolveDiscDecomp(p,t,epsilon,waveNumbers,xp,bgFlow,L,nParam, D, Re)
...
velocity = zeros(maxWaveNum,numPart, 2);
...
parfor waveIndex = 2:maxWaveNum
    ...
    buttshitp = zeros(numberOfNodes.old, numPart); %you need to create this because of how matlab does slicing
...
UhatNodes2(1:6,:,:) = UhatNodes2(particleNodes{i},:,:);
UhatNodes1(1:6,:,:) = UhatNodes1(particleNodes{i},:,:);
% Unodes2 = sum(real(UhatNodes2), 2)/maxWaveNum;
% Unodes1 = sum(real(UhatNodes1), 2)/maxWaveNum;
Unodes2 = cumsum(real(UhatNodes2(1:6,:,:)), 2)/maxWaveNum;
Unodes1 = cumsum(real(UhatNodes1(1:6,:,:)), 2)/maxWaveNum;
...
for i = 1:numPart
    ...
    velocity(:,i,1) = Unodes1' * PhiIPS/Re;
    velocity(:,i,2) = Unodes2' * PhiIPS/Re;
end