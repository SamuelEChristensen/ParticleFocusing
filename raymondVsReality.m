clear
pv = 4*[-0.5  -0.5; -0.5  0.5; 0.5  0.5;  0.5  -0.5;  -0.5  -0.5];
L = 10;
...
for hi=[0.015]
    ...
    fh=@(p) min(min(hi+max(0,1/3*drectangle(p, -0.001, 0.01, -0.001, 0.001).^2),0.08 + max(0,1/20*drectangle(p, -0.5, 0.5, -0.5, 0.5))),0.3);  % custom mesh distance that puts more points in one quadrant
