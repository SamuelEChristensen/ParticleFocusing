function [xp, velocities, eqPoints] = findMigrationPolyChannel(pv, Re, fhUser, xpUser, settings, varargin)
%% Calculation
narginchk(2,5)
...
if nargin==2
    ...
    [p,t]=distmesh2d(fd,@huniform,0.025*min(xlength,ylength),bbox,pv);
...
if nargin>2
    [p,t]=distmesh2d(fd,fhUser,0.02 ,bbox,pv);
...
fd=@(p) dpoly(p,pv);
ix = fd(p)<-0.04*max(xlength,ylength);
sp=p(ix,:);
