...
paramSet = {[0.01   0.005  220]  [0.01   0.005  420]};
xp = [2; 0.25];%[2*ones(size(0.1:0.05:0.45)); 0.1:0.05:0.45];
velocities = cell(length(paramSet), length(xp),2);
Ux = zeros(500,length(paramSet));
Uy = zeros(500,length(paramSet));
parami = paramSet{1};
fd=@(p) drectangle(p,0,4,0,1);
fh=@(p) min(max(parami(1), parami(1)+0.8*drectangle(p, 1.99, 2.01, 0.1, 0.5).^3),0.1);
...
tic
[u,p1,t1]=velocitySolveDiscDecomp(p,t, parami(2), waveNumbers, xp);
    for j = 1:length(1)
...
toc
    for j = 1:500
        xpi = [2; 0.5*j/500];
        xpn = whatTriangleIsThisPointIn(p1,t1,xpi);
        nodes = t1(xpn, :);
